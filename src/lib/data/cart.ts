...
/**
 * Saves delivery address + email for the cart, then auto-attaches the
 * appropriate shipping method so the cart is ready for payment.
 * Returns { ok: true } on success — the UI then redirects to payment.
 */
export async function saveDeliveryInfo(
  _currentState: unknown,
  formData: FormData
): Promise<{ ok: true } | string | null> {
  const cartId = await getCartId()
  if (!cartId) return "Coșul nu a fost găsit."
  ...
  const baseAddress = {
    ...
    address_2: (formData.get("cui") as string | null) ?? "",
  ...
  const deliveryMethod = (formData.get("delivery_method") as string | null) ?? ""
  const useDifferentBilling = formData.get("use_different_billing") === "true"

  // Build billing address — separate fields when user ticked "Detalii facturare"
  let billingAddress = baseAddress
  if (useDifferentBilling) {
    const billingFullName = (formData.get("billing_full_name") as string | null)?.trim() ?? ""
    ...
    billingAddress = {
      ...
      postal_code: (formData.get("billing_postal_code") as string | null) ?? "",
      country_code: "ro",
      phone: (formData.get("billing_phone") as string | null) ?? "",
    }
  }
  ...
  try {
    ...
    // Auto-attach the shipping method for both delivery types so Medusa's
    // validateShippingStep passes at order completion without a manual selection step.
    if (deliveryMethod === "pickup" || deliveryMethod === "livrare") {
      ...
      const option = isPickup
        ? shipping_options?.find((o) => o.name === "Ridicare Personală")
        : shipping_options?.find((o) => o.name === "Livrare Standard")

      if (!option) {
        return "Metoda de livrare nu are un preț configurat. Te rugăm să contactezi magazinul."
      }
...
/**
 * Attaches a chosen shipping method to the cart, then redirects to the payment step.
 */
export async function attachShippingMethod(
  _currentState: unknown,
  formData: FormData
): Promise<string | null> {
  const cartId = await getCartId()
  ...
  const optionId = formData.get("shipping_option_id") as string | null
  if (!optionId) return "Selectează o metodă de livrare."

  const countryCode = (formData.get("country_code") as string | null) ?? "ro"
  const deliveryMethod = (formData.get("delivery_method") as string | null) ?? "livrare"
